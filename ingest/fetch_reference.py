"""Pull reference geospatial data (address points, ward boundaries) into raw DuckDB tables.

These barely change day to day, but re-fetching them daily alongside the permits pull
keeps the pipeline simple (one script, no separate schedule) and costs little -- both
are narrowed to just the columns this project needs.
